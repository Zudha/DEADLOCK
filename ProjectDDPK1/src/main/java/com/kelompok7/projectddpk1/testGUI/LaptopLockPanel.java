...
public class LaptopLockPanel extends JPanel {
    ...
    public LaptopLockPanel(EscapeRoomGUI parent, Mode mode) {
        ...
        btnBack.addActionListener(e -> parent.room1());
        add(btnBack);
    }
    ...
    private void handleLockInput(KeyEvent e) {
        ...
        if (code >= KeyEvent.VK_0 && code <= KeyEvent.VK_9) {
            if (pinBuffer.length() < 8) { pinBuffer.append((char) e.getKeyChar()); feedbackMsg = ""; }
        } else if (code == KeyEvent.VK_BACK_SPACE) {
            if (pinBuffer.length() > 0) pinBuffer.deleteCharAt(pinBuffer.length() - 1);
            feedbackMsg = "";
        } else if (code == KeyEvent.VK_ENTER) {
            if (pinBuffer.toString().equals("04011937")) {
                cursorTimer.stop();
    ...
    private void drawLockScreen(Graphics2D g2, int sx, int sy, int sw, int sh) {
        ...
        String inst = "Tanggal koran (DDMMYYYY)";
        g2.drawString(inst, cx - g2.getFontMetrics().stringWidth(inst) / 2, sy + (int)(sh * 0.57));
        ...
        g2.drawString(pinBuffer.length() + "/8  [ENTER] konfirmasi", boxX, boxY + boxH + (int)(sh * 0.07));
    ...
    private void drawClueJamScreen(Graphics2D g2, int sx, int sy, int sw, int sh) {
        ...
        String[] narasi = { "Ada tiga jam di ruangan ini.", "Perhatikan polanya:" };
        ...
        g2.setFont(new Font("Monospaced", Font.BOLD, (int)(sh * 0.10)));
        int jamY = sy + (int)(sh * 0.44);
        String[] jams = { "01:00", "03:30", "06:00", "??:??" };
        int jamX = sx + (int)(sw * 0.04);
        int jamSpacing = (int)(sw * 0.24);
        for (int i = 0; i < jams.length; i++) {
            g2.setColor(i == 3 ? new Color(255, 220, 50) : new Color(50, 200, 255));
            g2.drawString(jams[i], jamX, jamY);
            if (i < 3) {
                g2.setFont(new Font("Monospaced", Font.PLAIN, (int)(sh * 0.08)));
                g2.setColor(new Color(150, 150, 150));
                g2.drawString("->", jamX + (int)(sw * 0.14), jamY - 2);
                g2.setFont(new Font("Monospaced", Font.BOLD, (int)(sh * 0.10)));
            }
            jamX += jamSpacing;
        }
